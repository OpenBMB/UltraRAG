...
The model is built on the [MiniCPM4.1](https://github.com/OpenBMB/MiniCPM4.1) base (8 billion parameters) and can accept user instructions to autonomously conduct in-depth research and generate long-form reports.

Key Highlights:

- **Exceptional Insight and Comprehensiveness**: As an 8B parameter edge-side model, AgentCPM-Report demonstrates remarkable potential in deep research report generation tasks, with performance that has surpassed some closed-source commercial agent systems. It redefines the performance ceiling for small-scale agents, achieving SOTA results particularly in the Insight metric.
- **Lightweight and Localized Deployment**: The model supports agile deployment on personal computers. By combining with frameworks like UltraRAG, users can build scalable local knowledge bases to generate reports that are more professional and in-depth than general-purpose large models. This lightweight and localized nature provides a secure and reliable foundation for deep report writing on private or proprietary data.
...
![](/images/demo/AgentCPM-Report.png)
